...
local function field(fp)
  ...
  if art:bands() == 3 then
    art = art:bandjoin{255}
  end
  field_pic = Fitter.cover(field_base, art, Layouts.field):composite(field_base,
                                                                     'over')
end
...
function shapes.anime.link_rating(link_rating)
  return TypeWriter.print(tostring(link_rating), get_base(), nil,
                          Layouts.anime.lkr)
end
...
local function fmt_sc(sc)
  return ('<span letter_spacing="-7168">%d</span>'):format(sc)
end
function shapes.proxy.pendulum_scales(lsc, rsc, size)
  local lt = TypeWriter.print(fmt_sc(lsc), get_base(), nil,
                              Layouts.proxy.lscale[size])
  return TypeWriter.print(fmt_sc(rsc), lt, nil, Layouts.proxy.rscale[size])
end

function shapes.proxy.pendulum_effect(effect, size)
  return TypeWriter.printf(effect, get_base(), nil,
                           Layouts.proxy.pendulum_effect[size])
end

function shapes.proxy.spelltrap_effect(effect)
  return TypeWriter.printf(effect, get_base(), nil,
                           Layouts.proxy.spelltrap_effect)
end
...
local desc_template = ([[<span letter_spacing="1024">%s%%s%s</span>]]):format(
                        brackets:format('['), brackets:format(']'))
local function fmt_desc(desc)
  return desc_template:format(desc)
end
function shapes.proxy.monster_desc(desc)
  return TypeWriter.print(fmt_desc(desc), get_base(), nil,
                          Layouts.proxy.monster_desc)
end
...
function shapes.proxy.monster_effect(effect)
  return
    TypeWriter.printf(effect, get_base(), nil, Layouts.proxy.monster_effect)
end
...
local function fmt_code(code)
  return code_template:format(code)
end
function shapes.proxy.serial_code(code, color)
  return TypeWriter.print(fmt_code(code), get_base(), color,
                          Layouts.proxy.serial_code)
end

local cr_template =
  [[<span letter_spacing="1024"><span size="5000">©</span>%s</span> %s]]
local default_year = 1996
local default_author = 'KAZUKI TAKASHI'
local function fmt_cr(year, author)
  return cr_template:format(year, author:upper())
end
function shapes.proxy.copyright(color)
  ...
  return TypeWriter.print(fmt_cr(year, author), get_base(), color,
                          Layouts.proxy.copyright)
end
