...
local function get_missing_custom_cols(cdb)
  local missing = custom_cols:copy()
  for col in cdb:nrows 'PRAGMA table_info(custom)' do
    rawset(missing, col.name, nil)
  end
  return missing
end
...
local function write_custom(cdb, entries)
  ...
  local add_sql = table.concat(cols
    :filter(function(c) return missing(c) ~= nil end, true)
    :map(function(c) return CUSTOM_ADDCOL_SQL:format(c, custom_cols[c]) end))
  cols = {'id'} .. cols
  local tuples = entries:map(function(e) return get_tuple(cols, e) end)
...
--- @param cdb Database database handle
--- @param ctables Fun list of names of SQLite tables to be cleaned
--- @param entries Fun entries that were written
local function clean_cdb(cdb, ctables, entries)
  local ids = table.concat(entries:map(fun 'e -> e.id'), ',')
  local dels = ctables:map(function(t) return DELETE_SQL:format(t, ids) end)
...
--- @param cdbfp string card database file path
--- @param entries Fun entries to be written
...
local function write_cdb(cdbfp, entries, overwrite)
  ...
  local sql = tables:map(function(cols, t)
    local tuples = entries:map(function(e) return get_tuple(cols, e) end)
    return get_replace_sql(t, cols, tuples)
  end):reduce('', fun 'a, b -> a .. b')
  local err = cdb:exec(sql)
  if err ~= sqlite.OK then
    Logs.warning(i18n 'make.writer.write_error', cdb:errmsg())
    return cdb:close()
  end
  local ctables = tables:vals()
  if write_custom(cdb, entries) then ctables:push('custom') end
  if overwrite then
    clean_cdb(cdb, ctables, entries)
  end
  cdb:close()
end
