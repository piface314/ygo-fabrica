...
return Decoder('proxy', Shapes.BASE, 'frame', {
  frame = function(card)
    ...
    if frame == 0 then return nil, i18n 'compose.decoder.no_card_type' end
  ...
  spelltrap = function(card, opts)
    ...
    local label = Layer(Shapes.ST_LABEL, st, st_type, opts.locale)
    local icon = Layer(Shapes.ATTRIBUTE, st, true, opts.locale)
  ...
  attribute = function(card, opts)
    local att = Parser.match_lsb(card.attribute, Codes.const.attribute.ALL)
    local layer = att > 0 and Layer(Shapes.ATTRIBUTE, att, false, opts.locale)
  ...
  forbidden = function(_, opts)
    return 'setnumber', Layer(Shapes.FORBIDDEN, opts.locale)
  end,
  ...
  seal = function(card, opts)
    ...
    local color = is_darkbg and WHITE or BLACK
    local year = card.year or opts.year or false
    local author = card.author or opts.author or false
    local locale = opts.locale
    local edition = Layer(Shapes.EDITION, pos, color, locale)
    local bevel = Layer(Shapes.OVERLAY, 'bevel')
    local copyright = Layer(Shapes.COPYRIGHT, year, author, color, locale)
