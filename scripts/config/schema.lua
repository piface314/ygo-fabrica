...
function validate.any(t, schema, strict)
  return validate[schema.type](t, schema, strict)
end
...
function validate.struct(t, schema, strict)
  ...
  for _, k in ipairs(schema.required or {}) do
    if t[k] == nil then
      ...
      struct[k] = schema.items[k].default
    end
  end
  for k, v in pairs(t) do
    if schema.items[k] then
      struct[k] = validate.any(v, schema.items[k], strict)
    end
  end
  return struct
end
...
function validate.string(t, schema, strict)
  return type(t) == 'string' and t or nil
end

function validate.boolean(t, schema, strict)
  return type(t) == 'boolean' and t or nil
end

function validate.number(t, schema, strict)
